...
2. Activity：用户收藏/关注策略后的观察列表、模拟/观察收益、策略信号、交易侧部署标记、端内消息流和 Toast。
...
算法开发：杨文园  
负责策略引擎服务，包括策略维护、策略定时回测、实时回测、部署策略自动化交易相关能力。策略引擎侧提供策略定义、可部署状态和策略运行事实，不负责向策略广场后端提供 Activity 部署标记聚合结果。

算法开发：丁宇杰  
负责算法策略执行引擎，包括生产策略信号、策略回测、策略观察收益快照、策略事件推送。策略执行侧把信号、异常、调仓、里程碑等事实事件提供给 Push 通道链路，消息入库和查询不在业务后端内实现。

后端业务开发：王正强  
负责策略广场服务，包括策略展示数据维护、策略展示列表、收藏/Activity、观察收益接口聚合、Push 订阅初始化、Push 消息通道对接、推荐接口对接和推荐兜底。业务后端不计算观察收益、不做算法失败兜底，不处理 Deployed 真实账户、持仓、交易明细，也不代理交易后端接口，不负责消息查询、详情、入库和已读状态。
...
前端开发：夏维森(Vincent)  
负责 Marketplace、Activity、Deployed 三个 tab 的页面实现和接口联调。Marketplace/Activity 调用策略广场后端，消息查询和端内实时消息连接 Push 服务，Deployed 及 Activity 部署标记调用交易后端。
...
| 策略展示信息维护 | 策略广场后端 | 后端业务 | 是 | 一期保留后端本地展示数据维护，不单列 Excel 导入任务模块 |
| 收藏/Activity | 策略广场后端 | 后端业务 + 算法 | 是 | 关注策略、观察收益、信号、消息；观察收益由算法接口返回 |
| Activity 部署标记 | 交易后端 | 交易 | 否 | 交易后端直接返回已部署标记、部署状态和入口 ID |
...
flowchart LR
    ...
    MKT -->|获取推荐策略 ID| REC
    MKT -->|策略定义 / 状态校验| ALG[策略引擎服务<br/>杨文园]
...
1. 策略基础展示信息：由后端本地维护，包括名称、标的、市场、策略简介、作者、标签、引擎、状态、Top/New 标记等；Hot/热度标签筛选一期不做。
...
2. `strategy_display` 是 Marketplace 展示的主数据来源，来自后端本地维护能力，保存策略名称、简介、标签、状态、风险提示等非实时展示字段。
...
| `QueryStrategyDefinitionStatus` | 策略引擎服务：杨文园 | 可选，用于展示前校验策略 ID 和策略侧状态 |
...
当前前端原型和职责边界下，一期仍由策略广场后端维护本地展示数据，但不单独建设 Excel 导入任务模块。`strategy_display` 只承载前端稳定展示字段，维护方式可以是后台录入、离线初始化或已有数据同步，本文不再限定导入实现。
...
| `parameter_summary` | `strategy_display.parameter_summary_json` | 参数摘要，详情页展示 |
...
| `top_flag / new_flag / publish_at` | `strategy_display.top_flag / new_flag / publish_at` | Top/New 和发布时间展示 |

不在本模块维护的数据：

| 数据类型 | 原因 | 来源 |
| --- | --- | --- |
| 策略收益、Sharpe、最大回撤、收益曲线 | 指标由算法统一计算，避免展示字段和计算字段混用 | `SyncStrategyMetricSnapshot` |
...
4. 当前方案不设计 `strategy_import_job`、行级错误明细和导入文件审计流程；如后续重新引入后台导入能力，再单独评审。
...
sequenceDiagram
    ...
    FE->>TRD: 查询 Activity 部署标记
    TRD-->>FE: strategy_id + deployment_status + deployment_id
```
...
4. Activity 部署标记不在策略广场接口内聚合，前端按 `strategy_id` 单独调用交易后端获取已部署状态和入口 ID。
5. 如果算法接口对部分策略返回失败，Activity 列表仍可返回策略卡片；该策略的 `metrics.calculationStatus` 应为 `failed`，前端展示失败态或重试入口。
...
| `deployment_id` | 交易后端部署标记返回 | 前端进入 Deployed 详情需要的入口 ID |
...
| `QueryActivityDeploymentMarkers` | 交易后端：丁骏(Jun Ding) | 前端按策略列表查询 Activity 部署标记 |

部署标记规则：

1. Activity 部署标记由交易后端直接提供给前端，不经过策略广场后端聚合。
2. 前端按 `strategy_id` 对齐 Activity 列表和部署标记结果。
3. 部署标记字段只表示“是否已部署、部署状态、部署入口/详情入口所需 ID”，不返回真实账户收益、账户号、持仓、订单或成交。
...
2. 如页面需要展示某策略是否已部署，前端按策略列表直接调用交易后端部署标记接口。
...
| 接口名 | 提供方 | 在本流程中的作用 |
| --- | --- | --- |
| `PushStrategySignalEvent` | 策略执行侧/交易侧 | 向业务后端提供策略或部署事实事件 |
| `DispatchPushChannelEvent` | Push 服务：李铎 | 策略广场后端调用 Push 服务通道接口，交由 Push 服务入库并投递 |
| `QueryPushMessages` | Push 服务：李铎 | 前端查询 Activity 右侧消息流和消息详情 |
...
4. Stop & Close 执行完成后，交易侧直接更新部署状态，并通过 Activity 部署标记接口返回最新结果；策略广场后端不负责把标记回流为 watching。
...
| `QueryActivityDeploymentMarkers` | 交易后端：丁骏(Jun Ding) | Activity 查询策略是否已部署和详情入口 ID |
| `QueryDeploymentPositions` | 交易后端：丁骏(Jun Ding) | 查询部署策略真实持仓 |
| `QueryDeploymentTrades` | 交易后端：丁骏(Jun Ding) | 查询部署策略订单、成交和交易明细 |
| `StopAndCloseDeployment` | 交易后端：丁骏(Jun Ding) | 停止策略并平仓，由交易侧维护后续部署状态 |
...
根据当前前端原型和最新职责边界，一期不单独建设 Excel 导入任务模块，因此策略广场业务后端当前不落地 `strategy_import_job` 表。

如后续重新引入后台导入能力，再单独评审以下内容：
...
| 接口名称 | 作用 | 使用方 | 提供方 | 状态 |
| --- | --- | --- | --- | --- |
| 查询当前用户已部署策略列表 | 原用于 Activity 后端聚合部署标记 | - | 杨文园 | 移出当前方案 |
| 查询策略定义状态 | 展示、部署入口校验使用 | 策略广场后端 | 杨文园 | 待提供 |
| 策略部署前校验 | Marketplace/Activity 进入 Deployed 前校验策略是否可部署 | 交易后端 | 杨文园 | 待提供 |

#### `QueryUserDeployedStrategies` 接口草案

该接口已移出当前一期技术方案。按照最新职责边界，Activity 部署标记由交易后端直接提供给前端，不再由策略引擎服务返回给策略广场后端做聚合。

#### `QueryStrategyDefinitionStatus` 接口草案

作用：展示或部署入口校验时，确认策略 ID 和策略侧状态是否存在。
...
| 推送策略信号事件 | 策略侧产生信号后通知业务后端做通道对接，由 Push 服务入库并投递 | 策略广场后端 | 丁宇杰 | 待提供 |
...
| 接口名称 | 作用 | 使用方 | 提供方 | 状态 |
| --- | --- | --- | --- | --- |
| Push 通道接口 | 策略广场后端按订阅关系提交推送事件，由 Push 服务入库并实时投递 | 策略广场后端 | 李铎 | 已有测试接口，正式协议待补充 |
| 查询消息列表/详情 | 前端查询 Activity 右侧消息流和消息详情 | 前端 | 李铎 | 待提供 |
...
5. Stop & Close 后的部署状态维护由交易后端负责，不回流到业务后端聚合。
...
| 查询 Activity 部署标记 | Activity 展示是否已部署、部署状态和详情入口 ID | 前端 | 丁骏(Jun Ding) | 待提供 |
...
3. Activity 通过交易后端接口获取部署标记，不经过策略广场后端聚合。
...
| 展示数据刷新 | 按运营维护节奏或离线同步触发 | 更新 `strategy_display` 本地展示字段 |
...
6. Activity 部署标记接口返回字段是否只需要 `strategy_id`、`deployment_id`、`deployment_status` 和入口类型。
7. 策略状态从 `listed` 到 `deployable` 的审批流程是否需要后台审核。
8. Deployed 的真实交易事件哪些需要进入 Activity 右侧消息流，由交易侧和 Push 侧单独定义事件范围。
